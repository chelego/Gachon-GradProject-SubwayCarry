...
- 김준 `role/player-gameplay` / `6406106`: 공용 플레이어·운반·균형 QTE·착석/넘어짐 애니메이션·내구도별 파손 표현.
...
2026-09-13부터 팀원 원본을 그대로 참조한다. 아트는 `Assets/_Project/Art/Sprites`, 플레이어는 `Scripts/Gameplay`, `Prefabs/Gameplay`, `Art/Concepts/Characters`, 공통 계약은 `Scripts/Core/Contracts`가 기준이다. `TeamReview` 복사본을 실행 의존성으로 사용하지 않는다. 통합용 설정과 연결 코드는 `ArtMap_GameplaySlice`에 유지한다. [팀 작업 방식](TEAM_WORKFLOW.md)을 참고한다.

기존 `Ai_v2`와 BasePassenger 프로필은 비교 기준으로 유지한다. 로컬에서 확인했던 03씬의 방향별 15명 설정도 보존한다. 통합 연결 변경은 NPC의 기본 성격·회피 수치를 변경하지 않는다.

## 검증과 미완성 범위

- 2026-09-07 당시 검사 기록: 행동/경로 32개, 실제 맵 90개 경로와 벽 205지점 검사 통과. 이후 병합의 재검증 결과와 혼동하지 않는다.
