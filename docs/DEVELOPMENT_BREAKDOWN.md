...
- 균형잡기 실패에 따른 넘어짐과 운반물 충격
- 케이크 상자와 케이크의 별도 내구도
- 승객과의 상대 충돌 속도에 따른 피해와 승객이 관여한 압박 피해
- 내구도에 따른 외형 단계 연결
...
- 탑승한 문과 관계없이 현재 객차의 출입문을 선택해 하차
- 하차 문 주변이 혼잡하면 여유로운 출입문으로 경로 변경
- 좌석, 기대는 위치, 손잡이 주변과 서 있는 위치 선택
- 빈 좌석 인식과 좌석으로 이동하는 행동
- 하차역 전에 미리 움직이거나 늦게 움직이는 차이
- 다른 승객과 플레이어를 피하며 목적지로 이동
- 진행 방향을 유지하는 근거리 회피와 막힌 일반 행동의 포기·재선택
- 승객 사이의 물리 밀림 방지와 필수 이동의 저속 비집기
- 상대 이동 방향을 반영한 마주침 예측 회피와 같은 방향 회피 시 빠른 방향 수정
- 승객별 보행속도와 하차 시 느긋함·뛰기 차이
- 문 앞 하차 대기열 배치와 대기 중 승객 간 거리 유지
...
| `Prototype_AI.unity` | 1번 객차 일반 승객 AI 20명의 랜덤 위치 탑승, 객차 행동과 하차 시험 |
...
- 배송 수락 시 케이크 상자 지급, 빈손/운반 자세·상자 분리 스프라이트 전환과 복귀 시 회수
- 케이크·상자 내구도, 스테미너와 상호작용 HUD
- 균형잡기 방향키, 진행도, 중앙·타이밍 안전 구간 표시
