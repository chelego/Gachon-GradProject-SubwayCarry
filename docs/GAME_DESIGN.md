...
본 프로젝트는 졸업프로젝트 AI 트랙으로 진행한다. 승객의 이동과 행동을 정해진 연출로만 처리하지 않고, 목적지, 성격, 혼잡도와 주변 상황에 따라 각 승객 AI가 판단하게 만든다.
...
| `Minimax` | 좌석 경쟁, 좁은 통로 진입과 문 앞 충돌처럼 선택이 맞부딪히는 짧은 상황에서 전진, 대기와 우회 판단 |
| `Local Avoidance / Steering` | 이동 중 다른 승객과 플레이어를 피하고 개인 공간과 진행 방향 조정 |

`Minimax`는 모든 승객의 기본 이동에 사용하지 않고, 상대의 다음 선택을 고려해야 하는 일부 경쟁 상황에 제한해서 적용한다.
