...
배송 확인 화면만 열었을 때는 플레이어가 물품을 들지 않는다. 배송을 수락하면 새 케이크 상자를 지급하고 운반 상태로 전환하며, 이후 개찰구에서 출발 교통비를 결제한다. 결제에 실패해도 수락한 배송과 물품은 유지하며, 배송 결과 정산 중에도 유지하고 가천대역 복귀 시 회수한다.
...
배송을 수락한 뒤에는 케이크 상자를 몸 앞에서 두 손으로 든다. 자유롭게 이동하고 회전할 수 있지만 균형 보호는 없다. 배송을 수락하기 전과 정산 후 복귀한 상태에서는 빈손 기본·이동 모습을 사용한다. 캐릭터의 운반 자세와 케이크 상자는 별도 스프라이트 레이어로 재생하고, 바라보는 방향에 따라 상자 레이어를 캐릭터 앞이나 뒤에 배치한다.
...
- 짧은 예고 뒤 QTE가 시작되며 진행 중에는 `WASD`를 이동 대신 중심잡기에 사용한다.
- 출발과 정차에는 관성의 반대 방향키를 연타해 게이지를 채운다.
- 운행 중 속도 변화에는 두 방향키로 표시를 안전 구간 중앙에 유지한다.
- 급정거에는 움직이는 표시가 안전 구간에 들어왔을 때 관성의 반대 방향키를 누른다.
- 잘못 누른 입력은 게이지 감소나 안전 구간 축소 같은 불이익을 주되 즉시 실패시키지 않는다.
- 제한시간이 끝나거나 중심을 안전 구간 밖에 오래 두면 넘어진다.
- 실패가 확정되면 넘어짐과 함께 운반물에 한 번의 충격 피해를 적용한다.
...
- 물건 Hitbox가 피해원으로 등록된 승객과 부딪힐 때 계산한다.
- 걷는 중 벽, 문, 좌석, 바닥이나 일반 사물에 닿는 것만으로는 피해를 받지 않는다.
- 승객과의 상대 충돌 속도가 빠를수록 피해가 커진다.
- 운반 상태 이름만으로 임의 피해 배율을 적용하지 않는다.
- 균형잡기에 실패하면 넘어짐과 함께 운반물에 한 번의 충격 피해를 적용한다. 이후 바닥이나 주변 사물 접촉으로 같은 실패 피해를 중복 적용하지 않는다.

### 10.2 압박 피해

물건이 승객과 다른 승객 또는 승객과 벽 사이에 끼면 압박을 계산한다. 물건이 들어갈 수 있는 간격이 원래 크기보다 좁아질수록 피해가 증가한다. 승객이 관여하지 않은 벽과 일반 사물 사이의 접촉만으로는 압박 피해를 받지 않는다. 구현이 불안정하거나 이해하기 어려우면 단순한 Overlap과 간격 계산으로 바꿀 수 있다.
...
HUD에는 운반물이 있을 때만 작은 상자 보호도와 큰 케이크 내구도 퍼센트를 표시한다. 운반물이 없을 때는 내구도 대신 `운반 물품 없음` 상태를 표시한다.
...
- 임시 열차 객차 두 칸
- 1번 객차에 동일한 일반 승객 AI 20명을 배치해 동시 탑승, 객차 행동 선택과 하차 시험
- 승객마다 현재 객차의 탑승 문을 랜덤으로 선택하고 탑승 위치가 겹치지 않게 임시 예약
- 빈 좌석, 좌석 앞 손잡이, 문 옆 기대기, 문 앞과 복도 중앙 위치
- 문 앞과 복도 입석은 고정점 주변의 작은 범위에서 실제 위치 선택
- 이동 방향 앞에 다른 승객이 있으면 좌우 중 여유로운 쪽으로 회피하고, 일반 행동 목적지가 계속 막히면 포기하고 다시 선택
- 승객끼리는 물리 힘으로 밀지 않으며 탑승과 하차처럼 포기할 수 없는 이동만 속도를 낮춰 약간 비집고 통과
- 마주 오는 승객의 이동 방향과 예상 위치를 보고 미리 피하며, 같은 쪽으로 피하면 한쪽이 빠르게 방향 수정
- 승객마다 느긋함·보통·빠름 이동속도를 다르게 적용하고 하차 시 천천히 이동하거나 뛰는 차이 추가
- 하차 준비를 시작한 위치에서 가장 가까운 사용 가능한 출입문으로 이동
- 문 앞 대기 위치는 하나의 점이 아닌 반경이 있는 구역으로 판정하고, 먼저 도착한 승객과 간격을 두고 대기
- `Utility AI` 가중치와 랜덤 보정을 사용한 행동 선택
- 앉기, 손잡이 잡기와 기대기는 높은 확률로 선택
- 문 앞과 복도 정지는 낮은 확률로 선택
- 처음 선택한 위치를 유지하고 새 빈 좌석이나 하차 준비처럼 이동할 이유가 생겼을 때만 행동 전환
- 서 있는 승객은 이전보다 빈 좌석이 늘어났을 때 일정 확률로 좌석 이동
- 30초 운행의 도착 직전에 출입문으로 이동해 다음 정차에서 하차
- 상자와 케이크 내구도
- 파란 좌석 한 칸당 2자리와 좌석 점유 상태
- 승객, 벽, 좌석과 닫힌 문 사이의 2D 충돌
- 두 객차 연결통로와 객차 단위 카메라 전환
- 임시 출입문 개폐와 승객 상태 표시
- 닫힌 출입문 패널은 객차 벽과 같은 두께와 중심선 사용
- 충돌과 압박 피해
- 균형잡기 3종
